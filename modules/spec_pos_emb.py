...
from x_transformers.x_transformers import FixedPositionalEmbedding, AbsolutePositionalEmbedding
...
class PatchyfiedSpecPosEmbs(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        temp_pos = torch.tensor(sum([[i % n]*n for i in range(T//n)], [])).view(-1, 1)
        freq_pos = torch.tensor(sum([list(range(0, n)) for _ in range(T//n)], []))#.view(-1, 1)
