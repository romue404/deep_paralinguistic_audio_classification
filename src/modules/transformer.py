...
class MySpecTf(ContinuousTransformerWrapper):
    ...
    def random_masking(self, lengths, p):
        ...
        mask_out_idxs = [
            random.sample(
                range(chunks[i], chunks[i] + lengths[i]),
    ...
    def forward(
        ...
        **kwargs
        ...
        x, intermediates = self.attn_layers(
            x,
            mask=self.make_mask(lengths, True).to(device),
            mems=mems,
            return_hiddens=True,
            **kwargs
        )
...
class MySpecTfMsmEncoder(MySpecTf):
    ...
    def forward(
        ...
        **kwargs
        ...
        spec_pos_emb = (
            self.pos_emb(data).expand(b, -1, -1).reshape(-1, d)
        )  # flatten pos embs
        keep_idxs = self.random_masking(lengths, 1.0 - mask_prob)  # idxs of kept tokens
        num_kept = torch.tensor([len(i) for i in keep_idxs])  # new lens
        keep_idxs = (
            torch.tensor(sum(keep_idxs, [])).squeeze().to(lengths.device)
        )  # flatten idxs of kept tokens
        ...
        cumsum_lens = [0] + num_kept.cumsum(
            0
        ).tolist()  # compute ranges for new sequences
        sequences = [
            x[i:j] for i, j in zip(cumsum_lens, cumsum_lens[1:])
        ]  # list of variable len sequences
        sequences = pad_sequence(sequences, batch_first=True)  # pad sequences

        # pass through transformer
        x, intermediates = self.attn_layers(
            sequences,
            mask=self.make_mask(num_kept, False).to(device),
            mems=mems,
            return_hiddens=True,
            **kwargs
        )
        ...
        kept_mask = (
            self.make_mask(num_kept, False).flatten().nonzero().squeeze()
        )  # indices of embeddings only
...
class MySpecTfMsmDecoder(MySpecTf):
    ...
    def forward(self, embeddings_flat, keep_idxs, old_lengths, **kwargs):
        ...
        spec_pos_emb = (
            self.pos_emb(x).expand(b, -1, -1).reshape(-1, self.pos_emb.dim)
        )  # flatten pos embs
        ...
        x, intermediates = self.attn_layers(
            x,
            mask=self.make_mask(old_lengths, False).to(device),
            mems=None,
            return_hiddens=True,
            **kwargs
        )
